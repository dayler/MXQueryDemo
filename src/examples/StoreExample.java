...
//		ctx.getStores().setUseFulltextStores(true);
	...
	private static void runFullTextStore() throws MXQueryException, IOException{
		final String query = "declare variable $data external; $data/books/book/title[. ftcontains 'Kill' case insensitive]";		
		Context ctx = new Context();
		CompilerOptions co = new CompilerOptions();
		co.setFulltext(true);
		XQCompiler comp = new CompilerImpl();
		...
		FullTextStore store = ctx.getStores().createFulltextStore("books.xml",
		        XDMInputFactory.createXMLInput(ctx, sr, false, Context.NO_VALIDATION,
